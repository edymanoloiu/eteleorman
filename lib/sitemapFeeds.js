...
async function loadPosts(fields) {
	return await getAllPosts(fields);
}

async function countNewsPosts() {
	const cutoff = Date.now() - NEWS_WINDOW_MS;
	const posts = await loadPosts(['slug', 'date', 'title', 'tags']);
...
export async function getActiveSitemapFeeds() {
	...
	if (built.length > 0) {
		...
		if ((await countRecentArticles()) > 0 && !feeds.includes('sitemap-articole.xml')) {
			feeds.push('sitemap-articole.xml');
		}
		if ((await countNewsPosts()) > 0) feeds.push('news-sitemap.xml');
